...
export default function TinZenOne({ activeTypeDefault = null }) {
	const { locale, asPath } = useRouter();
	const limit = 9;
	const list = ["web-news-zenone", "web-home-download"];
	...
	const dataZenOne = data?.filter((x) => x.ma_trang === "web-news-zenone")[0];

	// console.log("a", dataZenOne);
	const title =
		getFieldFromFieldId("title", "content_id", dataZenOne?.text_field, true)
			?.content_editor || "";
	...
	const seo = {
		title:
			dataZenOne?.seo_title ||
			"Tin tức ZenOne | Đầu tư bất động sản Blockchain",
		description: dataZenOne?.seo_description || "",
		picture1: dataZenOne?.seo_image_1 || null,
		picture2: dataZenOne?.seo_image_2 || null,
		url: asPath,
	};
			...
			<div>
				<Container>
					<Box pt={20}>
						...
						>
							<NewsSlider data={dataZenOne} isLoading={!data} />
						</Box>
						<NewsContainer>
							<Line index={0} />
							<Stack align={"center"}>
